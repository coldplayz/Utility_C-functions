/**
 * Note:
 * Initiate payment for bet utility product.
 *
...
export default class Bet {
  ...
  async processBet() {
    try {
      ...
      // Make payment
      const { customerId, amount, providerId } = this.data.paymentData;
      const initialStatus = await this.data.provider.bet(
        customerId,
        amount,
        providerId
      );
      ...
      else await updateUtilityTxStatusDocs(docsAccessData, { metadata });
