/**
 * Loads, parses, and exposes app environment configuration.
 *
...
export function validateEnv() {
  ...
  const envRules: { [k in keyof RawEnv]: string } = {
    ...
    NAIRA_PROJECT_SA: "required|string|max:10000",
    EBILLS_USERNAME: "required|string|max:100",
    EBILLS_PASSWORD: "required|string|max:100",
    EBILLS_BASE_URL: "required|string|max:100",
    EBILLS_AUTH_URL: "required|string|max:100",
    AUTHORIZED_SERVICES_ID: "present|string|max:10000",
    PUBSUB_JWT_SECRET: "required|string|max:1000",
    GIFTBILLS_API_KEY: "required|string|max:1000",
    GIFTBILLS_ENCRYPTION_KEY: "required|string|max:1000",
    GIFTBILLS_MERCHANT_ID: "required|string|max:1000",
    GIFTBILLS_BASE_URL: "required|string|max:1000",
