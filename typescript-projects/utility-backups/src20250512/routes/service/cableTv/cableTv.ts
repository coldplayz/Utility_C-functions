/**
 * Note:
 * Initiate payment for cableTv utility product.
 *
...
import { ProviderTxStatus, AppTxStatus } from "../../../interface/enums";
...
export default class CableTv {
  ...
  async processCableTv() {
    try {
      console.log("####### PaymentData", this.data.paymentData); // TODO: remove
      this.validateProcessData();

      // Make payment
      const { customerId, merchantId, planCode } = this.data.paymentData;
      const { initialStatus } = await this.data.provider.cableTv(
        customerId,
        merchantId,
        planCode!,
        this.data.appTxHash
