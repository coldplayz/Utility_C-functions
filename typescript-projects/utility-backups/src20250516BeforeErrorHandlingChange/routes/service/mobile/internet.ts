/**
 * Note:
 * Initiate payment for internet utility product.
 *
...
export default class Internet {
  ...
  async processInternet() {
    try {
      console.log("####### PaymentData", this.data.paymentData); // TODO: remove
      this.validateProcessData();

      // Make payment
      const { customerId, merchantId, productId } = this.data.paymentData;
      const { status, providerOrderId } = await this.data.provider.internet(
        customerId,
        merchantId,
        productId!,
...
const processDataRule = {
  ...
  "paymentData.productId": "required",
