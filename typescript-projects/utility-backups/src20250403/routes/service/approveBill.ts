...
export default class ApproveBill {
  data;
  ...
  async execute() {
    try {
      ...
      return "airtime request is approved!";
    } catch (error) {
      if (error instanceof ErrorHandler) throw error;
      throw new ErrorHandler("error", false, error);
    }
