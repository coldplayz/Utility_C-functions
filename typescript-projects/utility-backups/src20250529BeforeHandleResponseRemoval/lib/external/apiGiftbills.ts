...
export class Giftbills extends BaseProvider {
    ...
    if (/^dev/.test(process.env.NODE_ENV || "")) {
      ...
      try {
        ...
        const body = {
          ...
          reference: appOrderId, // + generateRandomIntRange(), // fix after test (remove random number)
        };
        ...
        const { data } = await axios({
          method: "post",
          url: new URL("/api/v1/betting/topup", parsedEnv.GIFTBILLS_BASE_URL)
            .href,
          data: body,
          headers,
        }).catch((error) => {
  ...
  async checkStatus(
    ...
    try {
      const { data } = await axios({
        method: "get",
        url: new URL(
          `/api/v1/bill/status/${providerOrderId}`,
  ...
  async getWalletBalance() {
    try {
      const { data } = await axios({
        method: "post",
        url: new URL(`/api/v1/check-balance`, parsedEnv.GIFTBILLS_BASE_URL)
          .href,
        headers: {
          Authorization: `Bearer ${parsedEnv.GIFTBILLS_API_KEY}`,
          MerchantId: parsedEnv.GIFTBILLS_MERCHANT_ID,
...
function getDummyPaymentResult(productId: string): IProviderPaymentResult {
  ...
  const token = productId === "electricity" ? "dummyToken" : undefined;
