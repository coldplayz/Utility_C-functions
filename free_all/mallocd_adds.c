	...
	for (i = 0; i < n; i++)
	{
		...
		switch (type[i])
		{
			case 'c':
				...
				for (j = 0; ptmt->ci <= 99; j++)
				{
					if (!ptmt->ptc[ptmt->ci] && (ptmt->ci <= 99))
					{
						if (type[i - 1] == '+')
						{
			...
			case 'i':
				...
				for (j = 0; ptmt->ii <= 99; j++)
				{
					if (!ptmt->pti[ptmt->ii] && (ptmt->ii <= 99))
					{
						if (type[i - 1] == '+')
						{
			...
			case 'p':
				...
				for (j = 0; ptmt->pi <= 99; j++)
				{
					if (!ptmt->pptc[ptmt->pi] && (ptmt->pi <= 99))
					{
						if (type[i - 1] == '+')
						{
